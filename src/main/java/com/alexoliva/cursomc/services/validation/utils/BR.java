package com.alexoliva.cursomc.services.validation.utils;
...
public class BR {
 ...
 private static final int [] weightSsn = {11, 10, 9, 8, 7, 6, 5, 4, 3, 2};
 	...
 	//CNPJ
 private static final int [] weinhtTin = {6, 5, 4, 3, 2, 9, 8, 7, 6, 5, 4, 3, 2};
 ...
 public static boolean isValidCPF(final String ssn) {
	 ...
	 return ssn.equals(ssn.subSequence(0, 9) + digit1.toString() + digit2.toString());	 
 }
 public static boolean isValidCNPJ(final String tin) {
	 if((tin == null) || (tin.length() != 14) || tin.matches(tin.charAt(0) + "{14}")) return false;
	 
	 final Integer digit1 = calculate(tin.substring(0, 12), weinhtTin);
	 final Integer digit2 = calculate(tin.substring(0, 12) + digit1, weinhtTin);
